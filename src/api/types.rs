...
mod tests {
    ...
    #[rstest]
    // Complete bitfield (0b11111111) - all 8 pieces available
    #[case(vec![0b11111111], 8, 0, 100, 100, true)] // First piece
    #[case(vec![0b11111111], 8, 0, 800, 100, true)] // All pieces
    #[case(vec![0b11111111], 8, 350, 100, 100, true)] // Middle of piece 3
    #[case(vec![0b11111111], 8, 700, 100, 100, true)] // Last piece
    #[case(vec![0b11111111], 8, 0, 1, 100, true)] // Single byte in first piece
    #[case(vec![0b11111111], 8, 799, 1, 100, true)] // Last byte
    // Partial bitfield (0b10101010) - pieces 1,3,5,7 available
    #[case(vec![0b10101010], 8, 100, 100, 100, true)] // Piece 1 available
    #[case(vec![0b10101010], 8, 300, 100, 100, true)] // Piece 3 available
    #[case(vec![0b10101010], 8, 500, 100, 100, true)] // Piece 5 available
    #[case(vec![0b10101010], 8, 700, 100, 100, true)] // Piece 7 available
    #[case(vec![0b10101010], 8, 0, 100, 100, false)] // Piece 0 unavailable
    #[case(vec![0b10101010], 8, 0, 200, 100, false)] // Spans pieces 0-1
    #[case(vec![0b10101010], 8, 200, 100, 100, false)] // Piece 2 unavailable
    #[case(vec![0b10101010], 8, 0, 800, 100, false)] // All pieces - some unavailable
    #[case(vec![0b10101010], 8, 50, 200, 100, false)] // Spans pieces 0-1
    // Edge case bitfield (0b11110000) - pieces 4-7 available
    #[case(vec![0b11110000], 8, 0, 0, 100, true)] // Empty range
    #[case(vec![0b11110000], 8, 500, 0, 100, true)] // Empty range at offset
    #[case(vec![0b11110000], 8, 400, 100, 100, true)] // Piece 4
    #[case(vec![0b11110000], 8, 0, 100, 100, false)] // Piece 0 unavailable
    // Multi-byte bitfield - 16 pieces, first 8 available
    #[case(vec![0b11111111, 0b00000000], 16, 0, 800, 100, true)] // Pieces 0-7
    #[case(vec![0b11111111, 0b00000000], 16, 100, 500, 100, true)] // Pieces 1-5
    #[case(vec![0b11111111, 0b00000000], 16, 350, 250, 100, true)] // Pieces 3-5
    #[case(vec![0b11111111, 0b00000000], 16, 700, 200, 100, false)] // Spans 7-8
    fn test_has_piece_range(
        #[case] bits: Vec<u8>,
        #[case] num_pieces: usize,
        #[case] offset: u64,
        #[case] size: u64,
        #[case] piece_length: u64,
        #[case] expected: bool,
    ) {
        let bitfield = PieceBitfield { bits, num_pieces };
        assert_eq!(
            bitfield.has_piece_range(offset, size, piece_length),
            expected
        );
    }

    #[rstest]
    #[case(0, 100, 0, false)] // Zero piece length with non-empty range
    #[case(0, 0, 0, true)] // Zero piece length with empty range
    ...
    #[test]
    fn test_has_piece_range_large_pieces() {
        let bitfield = PieceBitfield {
            bits: vec![0b00001111],
            num_pieces: 8,
        };
        let piece_length = 1024 * 1024; // 1MB pieces
