...
async fn test_concurrent_cache_readers() {
    ...
    assert_eq!(total_hits, total_reads as u64, "All reads should be cache hits");
    assert!(
...
fn test_inode_allocation_performance() {
    ...
    println!("Inode allocation performance: {} allocations/sec", throughput);
}
...
fn test_concurrent_inode_operations() {
    ...
    let total_allocated: usize = handles
        .into_iter()
        .map(|h| h.join().unwrap())
        .sum();
